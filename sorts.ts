import {
  ...
  engraver as DesignerType,
} from "@prisma/client";

type CheckListCoinType = {
  name: string;
  image: string | null;
  mintage: number | null;
  year: string;
...
export const CoinImageSort = (a: any, b: any) =>
...
export const CoinCommonNameSort = (a: CoinType, b: CoinType) =>
  a.common_name
    ?.toLocaleLowerCase()
    .localeCompare(b.common_name?.toLocaleLowerCase());

export const CoinCountryNameSort = (a: CoinType, b: CoinType) =>
  a.Country.name
    .toLocaleLowerCase()
    .localeCompare(b.Country.name.toLocaleLowerCase());

export const CoinCountryShortNameSort = (a: CoinType, b: CoinType) =>
  a.Country.short_name
    .toLocaleLowerCase()
    .localeCompare(b.Country.short_name.toLocaleLowerCase());

export const CoinStartYearSort = (a: CoinType, b: CoinType) =>
  a.year_start
    ?.toLocaleLowerCase()
    .localeCompare(b.year_start?.toLocaleLowerCase());

export const CoinNominalValueSort = (a: CoinType, b: CoinType) =>
...
export const CollectionCoinPurchaseDateSort = (
  a: CollectionType,
  b: CollectionType
) => {
  if (!a.sourced_when || !b.sourced_when) return 0;
  return a.sourced_when.localeCompare(b.sourced_when);
};

export const CollectionCoinPurchaseAmountSort = (
  a: CollectionType,
  b: CollectionType
) => {
...
export const CollectionCoinYearSort = (
  a: CollectionType,
  b: CollectionType
) => {
  if (!a || !a.year || !b || !b.year) return 0;
  const aInt = +a.year.match(/\d+/);
  const bInt = +b.year.match(/\d+/);
  return aInt < bInt ? -1 : 1;
};

export const CollectionCoinCountrySort = (
  a: CollectionType,
  b: CollectionType
) => {
...
export const CollectionCoinSellerSort = (
  a: CollectionType,
  b: CollectionType
) => {
  if (!a.sourced_from || !b.sourced_from) return 0;
