  ...
  return (
    <Container>
      <Box>
        ...
        <Typography variant="body2" gutterBottom>
          <Link href="/admin/ruler/create">Create new Ruler</Link>
        </Typography>
            ...
            <TableBody>
              {rulers.map((ruler) => (
                <TableRow>
                  ...
                  <TableCell>{moment(ruler.created_at).format('MMM DD, YYYY, HH:mm:ss')}</TableCell>
                  <TableCell>{moment(ruler.updated_at).format('MMM DD, YYYY, HH:mm:ss')}</TableCell>
                  <TableCell>
                    <Link href={`./edit/${ruler.id}`}>Edit</Link>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
