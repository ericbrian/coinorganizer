...
export default function page() {
  ...
  const filterImages = (imagesAlreadyUsed: { url: string }[], imagePool: string[]) => {
  ...
  async function getCoinsWithoutImages() {
    const endpoint = `${appconfig.envs[process.env.NODE_ENV].clientBaseUrl}/api/coins/withoutimages`;
    try {
  ...
  async function getImagesNames() {
    const endpoint = `${appconfig.envs[process.env.NODE_ENV].clientBaseUrl}/api/images/namesonly`;
    try {
      const res = await fetch(endpoint);
      return res.json();
    } catch (error) {
  ...
  const drop = (e: React.DragEvent) => {
    ...
    const preferred = eventTarget.id.split('-').at(-2) === 'PREFERRED' ? true : false;
    ...
    <>
      ...
      <Grid container spacing={2}>
            ...
            filteredImages.map((name) => (
              ...
              >
                <CardMedia
                  ...
                  image={appconfig.cdn + '/coin-images/' + name}
                  alt={name}
                  draggable="true"
        ...
        <Grid xs={6}>
          ...
          {!loading && images && coins.length === 0 && <div>No coins</div>}
          {coins.length > 0 &&
            coins.map((coin) => (
              <div
                style={{
